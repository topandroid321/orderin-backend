@extends('layouts.master_template')
    @section('content')
    <div class="py-1">
        <h1 class="h5">Edit Data Users</h1>
        <p>
                    ...
                    <form action="{{ route('users.update', $item->id) }}" class="w-full" method="post" enctype="multipart/form-data">
                        ...
                        <div class="flex flex-wrap -mx-3 mb-6">
                            <div class="w-full px-3">
                                ...
                                <input value="{{ old('email') ?? $item->email }}" name="email" class="appearance-none block w-full bg-gray-200 text-gray-700 border py-3 px-4 border-gray-200 leading-tight focus:bg-white focus:outline-none focus:border-gray-500" id="grid-last-name" type="email" placeholder="Email">
                            </div>
                        </div>
                        <div class="flex flex-wrap -mx-3 mb-6">
                            <div class="w-full px-3">
                                <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="grid-last-name">Username</label>
                                <input value="{{ old('username') ?? $item->username }}" name="username" class="appearance-none block w-full bg-gray-200 text-gray-700 border py-4 px-3 border-gray-200 leading-tight focus:bg-white focus:outline-none focus:border-gray-500" id="grid-last-name" type="text" placeholder="Username">
                            </div>
                        </div>
                        <div class="flex flex-wrap -mx-3 mb-6">
                            <div class="w-full px-3">
                                <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="grid-last-name">Phone</label>
                                <input value="{{ old('phone') ?? $item->phone }}" name="phone" class="appearance-none block w-full bg-gray-200 text-gray-700 border py-4 px-3 border-gray-200 leading-tight focus:bg-white focus:outline-none focus:border-gray-500" id="grid-last-name" type="text" placeholder="Phone">
                            </div>
                        </div>
                        <div class="flex flex-wrap -mx-3 mb-6">
                            <div class="w-full px-3">
